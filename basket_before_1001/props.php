<?
	...
	$arResult["ORDER_PROPS_PRINT"] = Array();
		...
		foreach ($arResult["ORDER_PROPS_PRINT"] as $item){
			...
			$tp++;
			if ($tp==1) echo '</div>';
			?><div style = "clear:left"><span class = "lrow"><label class = "timed" for = "<?=$item['CODE'].'_'.$item['ID']?>"><?=$item['NAME']?></label></span></div><?
			echo CalendarDate("".$item['CODE']."", "".date('d.m.Y')."", "", "15", "class=\"soa-property-container input-group\"");
			endif;
			if ($item['TYPE']=='RADIO'):
			...
			<input class = "radio" <?if ($item['REQUIED']=='Y'){?>required<?} ?> id = "<?=$item['CODE'].'_'.$arVal1['VALUE']?>" checked type = "radio" name = "<?=$item['CODE'];?>" value = "<?=$arVal1['VALUE']?>"><label for = "<?=$item['CODE'].'_'.$arVal1['VALUE']?>"><?=$arVal1['NAME']?></label><br>
			<input class = "radio" <?if ($item['REQUIED']=='Y'){?>required<?} ?> id = "<?=$item['CODE'].'_'.$arVal2['VALUE']?>" type = "radio" name = "<?=$item['CODE'];?>" value = "<?=$arVal2['VALUE']?>"><label for = "<?=$item['CODE'].'_'.$arVal2['VALUE']?>"><?=$arVal2['NAME']?></label>
			</div>
			<? endif;
				}
				endif;
