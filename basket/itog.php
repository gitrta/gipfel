...
$c3=array('Екатеринбург','Новосибирск');
	$cities = array_merge($c1,$c2);
	if (in_array($_GET['city'], $cities)) {
	    $DEP=0;
	    $flg = 1;
	}

	if (in_array($_GET['city'], $c3)) {
	    $flg = 0;
	}
}
...
if($GLOBALS["MCUR"] == "UAH"):
$ua = array_sum($arBasketItems);
$price = FormatCurrency($ua,'UAH');
$dost = $_GET['DEL_PRICE'];
$DEP = str_replace(' грн.', '', $_GET['DEL_PRICE']);
$itog = FormatCurrency($DEP+$ua,'UAH');
$flg=0;
endif;

if($GLOBALS["MCUR"] == "KZT"):
$ua = array_sum($arBasketItems);
$price = FormatCurrency($ua,'KZT');
$dost = $_GET['DEL_PRICE'];
$DEP = str_replace(' тг.', '', $_GET['DEL_PRICE']);
$DEP = str_replace(' ', '', $DEP);
//print_r($DEP);
$itog = FormatCurrency($DEP+$ua,'KZT');
$flg=0;
endif;
?>
