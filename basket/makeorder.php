...
<
if($GLOBALS["MCUR"]=='UAH'):
$dd = str_replace(' грн.', '', $_POST['sumdost']);
$_POST['sumdost'] = CCurrencyRates::ConvertCurrency(round($dd), $GLOBALS["MCUR"], "RUB");
$_POST['sumdost'] = round($_POST['sumdost']);
$_POST['sumdost'] = preg_replace('/[^0-9,]/', '', $_POST['sumdost']);
$SUMZAK = str_replace(' грн.', '', $_POST["SUMITOG"]);
$SUMZAK = str_replace(' ', '', $_POST["SUMITOG"]);
...
if(empty($email)){
	$arFieldss['EMAIL']='user_'.preg_replace("/[^0-9]/", '', $phone).'_'.rand(0,1000).'@gipfel.ru';
}
	
$UID = $user->Add($arFieldss);
}
if (empty($UID)) {echo $user->LAST_ERROR; die; } else{
	$USER->Authorize($UID);
}
...
if($GLOBALS["MCUR"] == 'UAH') {$prid = 3; $crts = 'UAH';}
...
if ($ar_res = $db_res->Fetch())
	{
		...
		if($crts=='UAH') $ar_res["PRICE"] = CCurrencyRates::ConvertCurrency($ar_res["PRICE"], "UAH", "RUB");
	   //print_r($ar_res);
...
if($crts=='KZT'):
$somePropValue = $propertyCollection->getItemByOrderPropertyId(5);
	$somePropValue->setValue('Алматы');
endif;
//print_r($order->getPropertyCollection()->getArray());
//die;
...
if($GLOBALS["MCUR"]=='UAH'):
$insZAK2 = "UPDATE `b_sale_order` SET `PRICE_DELIVERY` = '".$sumost."' WHERE `b_sale_order`.`ID` = ".$orderId;
$DB->Query($insZAK2, false, $err_mess.__LINE__);
endif;
echo $orderId;
//print_r($arFields);
