...
$query = http_build_query(array('q'=>$city));
$url = "http://api.cdek.ru/city/getListByTerm/jsonp.php?" . $query;
        $ch = curl_init(); 
...
$ctk=0;
	...
	$add=array(array("ID"=>30),array("ID"=>29));
	//print_r($add);
	$SD_CODES=array();
	$SP2 = array();
...
if($SCID){
	...
	$fromc = 44;
	if($GLOBALS['MCUR']=='KZT') $fromc = 4756;
	//print_r($fromc);
...
//var_dump($arResult["DELIVERY_LOCATION"]);
	...
	if ($DID!=='66'):
						...
						<div del_id = "<?=$DID;?>">
						<input required type="radio" class="desadasd" id="ID_DELIVERY_<?=$delivery_id?>_<?=$profile_id?>" name="<?=$arProfile["FIELD_NAME"]?>" value="<?=$delivery_id.":".$profile_id;?>" <?=$arProfile["CHECKED"] == "Y" ? "checked=\"checked\"" : "";?> />
...
?>
						...
						<div del_id = "<?= $arDelivery["ID"] ?>" >
							<input required type="radio" id="ID_DELIVERY_ID_<?= $arDelivery["ID"] ?>" name="<?=$arDelivery["FIELD_NAME"]?>" value="<?=$delivery_id.":".$profile_id;?>"<?if ($arDelivery["CHECKED"]=="Y") echo " checked";?>>
			...
			//endif;
		$tpku=0;	
foreach($FINALDESTINATIONS as $key=>$item){$tpku++; //echo $item['price'];
			?>
...
<?php if($arResult["DELIVERY_LOCATION"] == 241348) {?>
	<span class="flg" style="color: red;text-transform: uppercase;font-size: 16px;font-style: italic;text-align: center;display: block;top: 410px;right:160px;position:absolute;">Доставка для Вас бесплатна!</span>

<?php  }?>

<?php if($arResult["DELIVERY_LOCATION"] == 241390) {?>
...
if($arResult["DELIVERY_LOCATION"] == 89) {?>

			<div del_id="30">
	<input required="" type="radio" id="ID_DELIVERY_30" name="DELIVERY_ID" value="30">
		<label for="ID_DELIVERY_30">
			<span>СДЭК</span>
			<small><b>Доставка курьером</b></small>
		</label>
		<span class="toright">
							<b>350 <span>руб.</span></b>
							<text>Срок доставки: 2-3 дн.</text>
		</span>
</div>
...
<div del_id="29">
	<input required="" type="radio" id="ID_DELIVERY_29" name="DELIVERY_ID" value="29">
		<label for="ID_DELIVERY_29">
			<span>СДЭК</span>
			<small><b>Самовывоз, пункт выдачи</b></small>
		</label>
		<span class="toright">
							<b>210 <span>руб.</span></b>
							<text>Срок доставки: 2-3 дн.</text>
		</span>
</div>
...
<?php  if($arResult["DELIVERY_LOCATION"] == 241348 || $arResult["DELIVERY_LOCATION"] == 241390) {?>
	...
	<div del_id="81" class="del_moscow">
		<input required="" type="radio" id="ID_DELIVERY_81" name="DELIVERY_ID" value="81">
			<label for="ID_DELIVERY_81">
				<span>СДЭК</span>
				<small><b>Самовывоз, пункт выдачи</b></small>
			</label>
			<span class="toright">
