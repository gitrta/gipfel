...
if( empty($_POST['ONE_CLICK_BUY']['PHONE']) ) die(getJson(GetMessage('NO_PHONE')));
if( empty($_POST['ONE_CLICK_BUY']['NAME']) ) die(getJson(GetMessage('NO_USER_NAME')));
...
if($orderID){
	...
	while($arBasketItem = $resBasketItems->Fetch()){
		...
		if($arBasketItem['CAN_BUY'] === 'Y'){
			...
			$apr = str_replace('#', number_format($arBasketItem['PRICE'], $arCurrency["DECIMALS"], $arCurrency["DEC_POINT"], $currencyThousandsSep), $arCurrency['FORMAT_STRING']);
			$apr_1 = preg_replace('/[^0-9,]/', '', $apr);
			$apr_2 = FormatCurrency(CCurrencyRates::ConvertCurrency($apr_1, "RUB", $GLOBALS["MCUR"]),$GLOBALS["MCUR"]); 
			
			$apr0 = str_replace('#', number_format($arBasketItem['PRICE'], $arCurrency["DECIMALS"], $arCurrency["DEC_POINT"], $currencyThousandsSep), $arCurrency['FORMAT_STRING']);
			$apr_11 = preg_replace('/[^0-9,]/', '', $apr0);
			$apr_22 = FormatCurrency(CCurrencyRates::ConvertCurrency($apr_11, "RUB", $GLOBALS["MCUR"]),$GLOBALS["MCUR"]); 
			
			$orderList .= GetMessage('ITEM_NAME') . $arBasketItem['NAME']
				. GetMessage('ITEM_PRICE') . $apr_2
				. GetMessage('ITEM_QTY') . intval($arBasketItem['QUANTITY'])
				. GetMessage('ITEM_TOTAL') . $apr_22 . "\n";
		}
	}
	...
	$ddk = preg_replace('/[^0-9,]/', '', $arMessageFields['ORDER_PRICE']);
	$arMessageFields['ORDER_PRICE'] = FormatCurrency(CCurrencyRates::ConvertCurrency($ddk, "RUB", $GLOBALS["MCUR"]),$GLOBALS["MCUR"]); 
	//print_r($arMessageFields['ORDER_PRICE']);die;
