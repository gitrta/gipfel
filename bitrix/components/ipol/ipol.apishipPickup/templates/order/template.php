...
if(apishipHelper::isConverted()){
	$dTS = Bitrix\Sale\Delivery\Services\Table::getList(array(
		 ...
		 'filter' => array('CODE' => 'apiship:pickup')
	))->Fetch();
...
var IPOLapiship_pvz = {
	...
	arImages: {
		...
		"spsr" : "spsr-30px.png",
		"iml" : "iml-30px.png",
		"shoplogist" : "shoplogistic-30px.png",
		"qiwi" : "qiwi-30px.png",
	...
	onLoad: function(ajaxAns)
	{
		...
		<?if(COption::GetOptionString(CDeliveryapiship::$MODULE_ID,'pvzID',false)){?>
			tag = $('#<?=COption::GetOptionString(CDeliveryapiship::$MODULE_ID,'pvzID','')?>');
			if(tag.hasClass('apiship_custom_button'))
				tag.click(function(){ IPOLapiship_pvz.selectPVZ(); });
		<?}else{?>
			if (IPOLapiship_pvz.oldTemplate)
			{
				var parentNd=$('#'+IPOLapiship_pvz.deliveryLink);
				if(parentNd.closest('td', '#'+IPOLapiship_pvz.orderForm).length>0)
					tag = parentNd.closest('td', '#'+IPOLapiship_pvz.orderForm).siblings('td:last');
				else
					tag = parentNd.siblings('label').find('.bx_result_price');
			}
			else
			{
				if(!$('#IPOLapiship_injectHere').length)
					$('#bx-soa-delivery').find('.bx-soa-pp-company-desc').after('<div id="IPOLapiship_injectHere"></div>');
				
				if($('#IPOLapiship_injectHere').length == 0){
					$('#bx-soa-delivery .bx-soa-section-title-container').on('click',function(){IPOLapiship_pvz.onLoad();});
					$('#bx-soa-delivery .bx-soa-editstep').on('click',function(){IPOLapiship_pvz.onLoad();});
					$('#bx-soa-region .pull-right').on('click',function(){IPOLapiship_pvz.onLoad();});
				}
				
				tag = $('#IPOLapiship_injectHere');
			}
		<?}?>
		...
		else
		{
			if (newTemplateAjax)
			{
				// console.log(ajaxAns.apiship.apiship_city);
	...
	choozePVZ: function(pvzId,isAjax){// выбрали ПВЗ
		...
		IPOLapiship_pvz.pvzAdress=IPOLapiship_pvz.city+", "+IPOLapiship_pvz.PVZ[pvzId]['Address']+" #S"+IPOLapiship_pvz.PVZ[pvzId].code;
