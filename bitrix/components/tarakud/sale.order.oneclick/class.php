...
class CSimpleComponent extends CBitrixComponent
{
	...
	public function onPrepareComponentParams($params)
	{
		...
		$params["CONFIRM_ORDER"] = trim($params["CONFIRM_ORDER"]);
	...
	protected function addOrder()
	{
		...
		if (empty($this->arResult["ERROR_MESSAGE"]))
		{
			...
			//if ($option == "Y")
			//	COption::SetOptionString("main", "~sale_converted_15", "N");
			...
			else
			{
				...
				if ($ORDER_ID > 0)
				{
					//CSaleBasket::OrderBasket($ORDER_ID, 0, $arOrder["SITE_ID"], false);
			...
			if ($ORDER_ID > 0 && empty($arErrors))
			{
				...
				//update basket order
				if ($this->arParams["ORDER_PRODUCT"] == 2 || $this->arParams["ORDER_PRODUCT"] == 3)
				{
