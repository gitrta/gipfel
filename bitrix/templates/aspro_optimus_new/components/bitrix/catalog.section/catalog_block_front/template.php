					...
					<div class="inner_wrap">
						<div class="image_wrapper_block">
							...
							<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="thumb">								
								...
								?>
								<?if( !empty($arItem["PREVIEW_PICTURE"]) ): $img["src"] = $arItem["PREVIEW_PICTURE"]['SRC'];?>
								<?$img = CFile::ResizeImageGet($arItem["PREVIEW_PICTURE"], array( "width" => 282, "height" => 202 ), BX_RESIZE_IMAGE_PROPORTIONAL,true,false,false,85 );?>
									<img class="noborder prp" src="<?=$img["src"]?>" alt="<?=$a_alt;?>" title="<?=$a_title;?>" />
									<div class = "backim" style = "background: url('<?=$img["src"]?>')" ></div>
								<?elseif( !empty($arItem["DETAIL_PICTURE"])):?>
								<?$img = CFile::ResizeImageGet($arItem["DETAIL_PICTURE"], array( "width" => 282, "height" => 202 ), BX_RESIZE_IMAGE_PROPORTIONAL,true,false,false,85 );?>
									<div class = "backim" style = "background: url('<?=$img["src"]?>')" ></div>
									<img class="noborder dtp" src="<?=$img["src"]?>" alt="<?=$a_alt;?>" title="<?=$a_title;?>" />
								<?else:?>
						...
						<div class="item_info">
										...
										<?if($arPrice["CAN_ACCESS"]){
											...
											<?}else{?>
												<div class="price only_price">
													...
													<?if(strlen($arPrice["PRINT_VALUE"]) > 0) { ?>
							...
							<? if ($arQuantityData['TEXT']=='Есть в наличии'):?>
							<div class="footer_button">
								<div class="counter_wrapp">
									<div class="button_block">
										<!--noindex-->
											<!--<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="button basket read_more"><?=GetMessage("CATALOG_READ_MORE");?></a>-->
											<span class="small to-cart button transition_bg" data-item="<?=$arItem["ID"]?>" data-float_ratio="" data-ratio="1" data-bakset_div="bx_basket_div_<?=$arItem["ID"]?>" data-props="" data-part_props="Y" data-add_props="Y" data-empty_props="Y" data-offers="" data-iblockid="2" data-quantity="1">
											<i></i>
											<span>В корзину</span>
											</span>
...
<!--										
<span class="transparent big_btn type_block button one_click myone" 
											data-item="<?=$arItem["ID"]?>" data-iblockid="2" data-quantity="" onclick="yaCounter39898360.reachGoal('25145004')">-->


<span class="transparent big_btn type_block button one_click myone" 
											data-item="<?=$arItem["ID"]?>" data-iblockid="2" data-quantity="" onclick="oneClickBuy('<?=$arItem["ID"]?>', '2', this)">
