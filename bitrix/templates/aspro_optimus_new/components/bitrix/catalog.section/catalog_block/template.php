...
<?if( count( $arResult["ITEMS"] ) >= 1 ){?>
	...
	<?}?>
		...
		<? $itr=0;foreach($arResult["ITEMS"] as $arItem){$itr++;?>
			<div class="item_block col-<?=$col;?>">
				<div class="catalog_item_wrapp item">
					...
					?>
					<div class="catalog_item item_wrap <?=(($_GET['q'])) ? 's' : ''?>" id="<?=$arItemIDs["strMainID"];?>"  style = "min-height:373px;">
						<div>
							<div class="image_wrapper_block">
								...
								<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="thumb" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>">
										...
										<?if( !empty($arItem["PREVIEW_PICTURE"]) ):?>
											<img class = "jno" border="0" src="<?=$arItem["PREVIEW_PICTURE"]["SRC"]?>" alt="<?=$a_alt;?>" title="<?=$a_title;?>" />
											<div class = "backim" style = "background: url('<?=$arItem["PREVIEW_PICTURE"]["SRC"]?>');"></div>
										...
										<?else:?>
											...
											<div class = "backim" style = "background: url('<?=SITE_TEMPLATE_PATH?>/images/no_photo_medium.png');"></div>
										<?endif;?>
									</a>
							</div>
							<div class="item_info <?=$arParams["TYPE_SKU"]?>">
										...
										<?foreach($arItem["PRICES"] as $key => $arPrice){?>
											<?if($arPrice["CAN_ACCESS"]){
												...
												<?}else{?>
													...
													if($arItem['DISPLAY_PROPERTIES']['OLD_PRICE']['VALUE']>0 && intval($arItem['DISPLAY_PROPERTIES']['OLD_PRICE']['VALUE']) > intval($arPrice['DISCOUNT_VALUE_VAT'])) echo "<div class = 'price discount'><span>".FormatCurrency(CCurrencyRates::ConvertCurrency($arItem['DISPLAY_PROPERTIES']['OLD_PRICE']['VALUE'], "RUB", $GLOBALS["MCUR"]),$GLOBALS["MCUR"])."</span></div>";?>
								...
								<div class="wrapp_one_click custone">
									<span class="transparent big_btn type_block button one_click myone" data-item="<?=$arItem["ID"]?>" data-iblockID="<?=$arParams["IBLOCK_ID"]?>" data-quantity="<?=$arParams["MIN_QUANTITY_BUY"];?>" onclick="oneClickBuy('<?=$arItem["ID"]?>', '<?=$arParams["IBLOCK_ID"]?>', this);yaCounter39898360.reachGoal('pokupka1klic');">
										<span>Купить в 1 клик<?//=GetMessage('ONE_CLICK_BUY')?></span>
									</span>
								</div>
								</div>
							</div>
...
// Добавить в корзину
 $(document).on("click", ".catalog_item .to-cart", function() {
	idCat = $(this).attr('data-item');
	priceCat = parseInt($(this).closest(".catalog_item").find(".prices .price").text().replace(/\D+/g,""));

		setTimeout(function() {
				dataLayer.push({
				 'goods_id': idCat,
				 'goods_price': priceCat,
	...
	$(".catalog_item").each(function() {
		idCat = $(this).find(".to-cart").attr('data-item');

		if(idCat != undefined && prodCounter < 3 ) {
...
setTimeout(function() {
		dataLayer.push({
		 'goods_id': catArr,
		 'event': 'pixel-mg-event-category',
		 'pixel-mg-event-category': 'Enhanced Ecommerce',
		 'pixel-mg-event-action': 'Category Product Details',
		 'pixel-mg-event-non-interaction': 'True',
		});
}, 2000);

// Купить в один клик
$(document).on("click", ".catalog_item .one_click", function()  {
	idCat = $(this).attr('data-item');
	priceCat = parseInt($(this).closest(".catalog_item").find(".prices .price").text().replace(/\D+/g,""));
});
...
/*		$('.catalog_block .catalog_item_wrapp .catalog_item .item-title').sliceHeight();
		...
		#catalog_item item_wrap hover shadow_delay
*/
	});
