...
<script type="text/javascript" src="/bitrix/components/bitrix/catalog.product.subscribe/templates/.default/script.js?151375701616889"></script>
...
$nums = preg_replace('/[^0-9]/', '', $arResult["MIN_PRICE"]["PRINT_VALUE"]);
  ...
  'ecommerce': {
    'detail': {
      'products': [{
        'name': '<?=$arResult["NAME"];?>',     
       	'id': '<?=$arResult["ID"];?>',          
       	'price': '<?=(int) $nums;?>',              // Тип: Number
       	'brand': '',      // Название бренда (Reima, Kerry...). При отсутствии бренда выводить пустое значение
       	'variant': '',     
       }]
     }
   }
			...
			<div class="slides">
					...
					<div class="offers_img wof">
						...
						<?if($arFirstPhoto["BIG"]["src"]){?>
							<a href="<?=($viewImgType=="POPUP" ? $arFirstPhoto["BIG"]["src"] : "javascript:void(0)");?>" class="<?=($viewImgType=="POPUP" ? "popup_link" : "line_link");?>" title="<?=$title;?>">
								<img id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>" src="<?=$arFirstPhoto['BIG']['src']; ?>" <?=($viewImgType=="MAGNIFIER" ? 'data-large=""': "");?> alt="<?=$alt;?>" title="<?=$title;?>">
							</a>
						<?}else{?>
				...
				<?}else{
					if($arResult["MORE_PHOTO"]){?>
						<ul>
							<?
								if(empty($arResult['PROPERTIES']['YOUTVID']['VALUE'])): $CL = 'class="current"'; else: $CL = ''; endif;
								if(!empty($arResult['PROPERTIES']['YOUTVID']['VALUE'])): $u1 = parse_url($arResult['PROPERTIES']['YOUTVID']['VALUE'], PHP_URL_QUERY); $vID = str_replace('v=','',$u1);?>
								<li class="current" id="photo-vid"> 
<iframe style = "width:100%;" height="340" src="https://www.youtube.com/embed/<?=$vID; ?>" frameborder="0" allowfullscreen></iframe>
								</li>
								<? endif; ?>
							<?	foreach($arResult["MORE_PHOTO"] as $i => $arImage){?>
								...
								$title=$arImage["TITLE"];
								?>
								<li id="photo-<?=$i?>" <?=(!$i ? $CL : 'style="display: none;"')?>>
									<?if(!$isEmpty){?>
										<a href="<?=($viewImgType=="POPUP" ? $arImage["BIG"]["src"] : "javascript:void(0)");?>" <?=($bIsOneImage ? '' : 'data-fancybox-group="item_slider"')?> class="<?=($viewImgType=="POPUP" ? "popup_link fancy" : "line_link");?>" title="<?=$title;?>">
											<img  src="<?=$arImage["BIG"]["src"]?>" <?=($viewImgType=="MAGNIFIER" ? "class='zoom_picture'" : "");?> <?=($viewImgType=="MAGNIFIER" ? 'data-large="'.$arImage["BIG"]["src"].'"': "");?> alt="<?=$alt;?>" title="<?=$title;?>" />
										</a>
									<?}else{?>
					...
					<div class="wrapp_thumbs">
						<div class="thumbs flexslider" data-plugin-options='{"animation": "slide", "selector": ".slides_block > li", "directionNav": true, "itemMargin":10, "itemWidth": 54, "controlsContainer": ".thumbs_navigation", "controlNav" :false, "animationLoop": true, "slideshow": false}' style="max-width:<?=ceil(((count($arResult['MORE_PHOTO']) <= 4 ? count($arResult['MORE_PHOTO']) : 4) * 64) - 10)?>px;">
							<ul class="slides_block" id="thumbs">
								<?	if(!empty($arResult['PROPERTIES']['YOUTVID']['VALUE'])):?>
								<li>
										<span><img src="/upload/youtube-play-png.jpg" /></span>
									</li>
								<? endif; ?>
								<?foreach($arResult["MORE_PHOTO"]as $i => $arImage):?>
									<li <?=(!$i ? 'class="current"' : '')?>>
...
<iframe style = "width:100%;" height="340" src="https://www.youtube.com/embed/<?=$vID; ?>" frameborder="0" allowfullscreen></iframe>
					...
					<?if($arResult["MORE_PHOTO"]){
						foreach($arResult["MORE_PHOTO"] as $i => $arImage){?>
							<?$isEmpty=($arImage["SMALL"]["src"] ? false : true );?>
							<li id="mphoto-<?=$i?>" <?=(!$i ? $CL : 'style="display: none;"')?>>
								<?
								$alt=$arImage["ALT"];
								$title=$arImage["TITLE"];
								?>
							...
							<?foreach($arResult["PRICES"] as $key => $arPrice){?>
								<?if($arPrice["CAN_ACCESS"]){
									...
									<?if($arPrice["VALUE"] > $arPrice["DISCOUNT_VALUE"] && $arParams["SHOW_OLD_PRICE"]=="Y"){?>
										...
										<?}*/?>
									<?}else{?>
										<div class="price eres" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PRICE']; ?>">
											<?if(strlen($arPrice["PRINT_VALUE"])):?>
<?php $_SESSION['dfdfd'] = round($arPrice["VALUE_VAT"]) ?>
								...
								<?}?>
							<?}?>
							<?if(($arResult["PRICES"]["BASE"]["VALUE"] == $arResult["PRICES"]["BASE"]["DISCOUNT_VALUE"]) && $arResult["PROPERTIES"]["OLD_PRICE"]["VALUE"]):?>
								<?="<div class = 'price discount'><span>".FormatCurrency(CCurrencyRates::ConvertCurrency($arResult["PROPERTIES"]["OLD_PRICE"]["VALUE"], "RUB", $GLOBALS["MCUR"]),$GLOBALS["MCUR"])."</span></div>";?>
							<?endif;?>
						<?}?>
					</div>
...
var price = parseInt($(".prices_block .prices .price:first").text().replace(/\D+/g,""));
var pageT = '<?=$arResult['NAME']?>';
var IDi = '<?=$arResult['ID']?>';
...
$(document).on("click", ".item_main_info .to-cart", function() {
...
$(document).on("submit", "#one_click_buy_form", function() {
		setTimeout(function() {
			dataLayer.push({
					 'goods_id': '<?=$arResult['ID']?>',
					 'goods_price': price,
...
// Добавление в корзину товаров ниже
$(document).on("click", ".catalog_item .to-cart", function() {
