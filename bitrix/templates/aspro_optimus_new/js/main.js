...
$(document).ready(function($){
	var shir = $(window).width();
	var sct = 115;
	if (shir<569){
		sct=104;
	}
	var vys = '';
    var nav = $('.menu_colored');

    $(window).scroll(function () {
        if ($(this).scrollTop() > sct) {
	        vys = $('#header .wrapper_inner').outerHeight();
			vys = vys+nav.height();
	        $('#header').css('height',vys+'px');
	        $('.menu_colored .wrapper_inner').css('background','#6286c3');
            nav.addClass("fixm");
        } else {
	        $('#header').css('height','auto');
	        $('.menu_colored .wrapper_inner').css('background','white');
            nav.removeClass("fixm");
        }
    });
});
	...
	if($('.zoom_picture').length){
		...
		var zoomer = $('.zoom_picture'),
			...
			defaults = {
				...
				magnifiersize:[sw,slide.height()-1],
				//magnifiersize:[sw,sw],
				//rightoffset:30,
				rightoffset:1,
				topoffset:1,
...
function activate_menu_block(block) {
  if ($(block).hasClass('wait-active')) {
    $(block).addClass('active');
  //  $(block).parent().addClass('hidded');
  }
}
...
$(document).ready(function(){
  ...
  $(".menu_top_catalog_li")
    .mouseenter(function() {
      ...
      setTimeout(activate_menu_block, 300, this);
    })
    .mouseleave(function() {
			...
			if(arOptimusOptions["PRICES"]["MIN_PRICE"]){
				if(arOptimusOptions["PRICES"]["MIN_PRICE"]>summ){
					...
					if(!$('.oneclickbuy.fast_order').length && arOptimusOptions["THEME"]["SHOW_ONECLICKBUY_ON_BASKET_PAGE"] == "Y")
						$('.bx_ordercart_order_pay_center').append('<span class="oneclickbuy button big_btn fast_order transparent big_btn type_block button one_click myone" onclick="oneClickBuyBasket()">Купить в 1 клик</span>');
