...
<div class="section" <?if(count($arResult["PAY_SYSTEM"])<2):?> style="display:none"<?endif?>>
	...
	<?if($arResult["PAY_FROM_ACCOUNT"] == "Y" ||  count($arResult["PAY_SYSTEM"])>0):?><div class="bx_section">
        ...
        /*
		if($arResult["ORDER_PROP"]["USER_PROPS_N"][6]["VALUE"] == "671" or $arResult["ORDER_PROP"]["USER_PROPS_N"][6]["VALUE"] == "672" or $arResult["ORDER_PROP"]["USER_PROPS_N"][6]["VALUE"] == "673")
		{
			unset($arResult["PAY_SYSTEM"][0]);
		}
        */
		...
		foreach($arResult["PAY_SYSTEM"] as $key=>$arPaySystem)
		{
			...
			if (strlen(trim(str_replace("<br />", "", $arPaySystem["DESCRIPTION"]))) == 0 && intval($arPaySystem["PRICE"]) == 0)
			{
				...
				{
					?>
					<div class="bx_block horizontal hide hidden">
						<div class="bx_element">
							<input type="hidden" name="PAY_SYSTEM_ID" value="<?=$arPaySystem["ID"]?>">
							<input type="radio"
								id="ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>"
				...
				{
					if($arResult["ORDER_PROP"]["USER_PROPS_N"][6]["VALUE"] == "671"){
