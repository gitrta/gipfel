...
if (!empty($arResult["ORDER"]))
{
	...
	if ($_SESSION['flag']!=$arOrder['ID'] && $SESSION_FLAG!=$arOrder['ID']){
		if (CModule::IncludeModule("sozdavatel.sms")){ 
			...
			if ($phone)  
			{
				$message =  $vars["PROP_NAME"]." ".$vars["PROP_LASTNAME"].", Ваш заказ обработан! Номер заказа: ".$arOrder['ID'].". Сумма ".$vars['PRICE']." Доставка (".$vars["PROP_F_DATE"]." ".$vars["PROP_F_TIME"].") Курьер за час с Вами свяжется".$arOrder['ID'].". gipfel.ru";
	...
	if (empty($_SESSION['GPFL_ORDERS'][$arOrder['ID']])):
	?>
	<script>SendAnalyticsGoal('zakaz_oformlen');</script>
	<?endif?>
            <h1 class="product_title">Заказ сформирован1</h1>
            ...
            <div class="order_done_block clearfix">
                ...
                <div class="order_done_block_right">
                    ...
                    <h2>В ближайшее время на оператор свяжется с Вами</h2>
                    <div class="order_done_top">                        
                        Уважаемый <strong><?=$vars['PROP_NAME']?> <?=$vars['PROP_LASTNAME']?></strong>, благодарим Вас за заказ в Gipfel.ru!<br/>
                        Номер Вашего заказа: <strong><?=$arResult['ORDER']['ID']?></strong> <?=$arOrder['DATE_INSERT_FORMAT']?><br/>
                        ...
                        <div class="cart_table">
                            ...
                            <?/*<div class="cart_table_item">
                                <div class="cart_table_item_product">
                                    ...
                                    <div class="cart_table_item_product_info">
                                        <div class="cart_table_item_product_info_title"><a href="/surprize.html">Gipfel</a></div>
                                        <div class="cart_table_item_product_description">
                                            Оформите заказ свыше 5000 рублей и получите подарок - бесплатная доставка
                                        </div>
                                    </div>
                                </div>
                                <div class="cart_table_item_count">
...
<?
$_SESSION['GPFL_ORDERS'][$arOrder['ID']] = 'Y'; // чтобы при обновлении страницы не происходила отправка цели заново
?>