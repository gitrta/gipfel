...
if ($arResult['DETAIL_PICTURE']) array_unshift($arResult['MORE_PHOTO'],$arResult['DETAIL_PICTURE']);
...
?>
	<div class="bx_item_container product clearfix">
		<div class="product_left">
                <div class="product_big_photo">	
				
					<div class="ribbons">
						<?if($arResult['PROPERTIES']['NOVINKA']['VALUE']=="НОВИНКА"){?>							                            														
						<span class="novinka"></span>
						 <?}?>
						<?if($arResult['PROPERTIES']['NOVINKA']['VALUE']!="НОВИНКА" and $arResult['PROPERTIES']['AKCII']['VALUE']=="Акция 1+1"){?>							                            														
						<span class="a11"></span>
						 <?}?>		
			...
			<span class="bx_bigimages_aligner">
			  <a rel='gal1' class="jqzoom" href="<? echo $arFirstPhoto['SRC']; ?>">
				<img
				id="<? echo $arItemIDs['PICT']; ?>"
				src="<? echo $arFirstPhoto['SRC']; ?>"
				alt="<? echo $strAlt; ?>"
				title="<? echo $strTitle; ?>" itemprop="image">
			  </a>
			</span>	
					<?/*$APPLICATION->IncludeComponent("mdsoft:retinazoom-for-eshop", "template1", Array(
						"INCLUDE_JQUERY" => "Y",	// Подключить библиотеку jQuery
						"BOX_SIZE" => "350",	// Размеры рамки
						"IMAGE_WIDTH" => "",	// Ширина изображения
						"IMAGE_HEIGHT" => "",	// Высота изображения
						"IMAGE_ALT" => $strAlt,	// Тэг Alt
						"IMAGE_TITLE" => $strTitle,	// Тэг Title
						"ELEMENT_URL" => $arFirstPhoto["SRC"],	// URL изображения
						"IMAGE_ID" => $arItemIDs["PICT"],	// Уникальный ID для изображения
...
?>
	<div class="product_photos" id="<? echo $arItemIDs['SLIDER_CONT_ID']; ?>">

<?
		foreach ($arResult['MORE_PHOTO'] as $k=>&$arOnePhoto)
		{
?>
                        <div class="product_small_photo<?if(!$k):?> active<?endif?>">
                            <a rel="{gallery: 'gal1', smallimage: '<? echo $arOnePhoto['SRC']; ?>',largeimage: '<? echo $arOnePhoto['SRC']; ?>'}" href="<? echo $arOnePhoto['SRC']; ?>"><img src="/bitrix/images/1.gif" style="background:url(<? echo $arOnePhoto['SRC']; ?>) no-repeat center; background-size:contain;" alt=""></a>
                        </div>
...
?>
                ...
                <div class="product_right">
					...
					<div class="product_code">
					                    Код товара: <?=$arResult['PROPERTIES']['ARTNUMBER']['VALUE']?><br/>
										<?if($arResult['PROPERTIES']['SERIES']['VALUE']):?>Серия: <?=$arResult['PROPERTIES']['SERIES']['VALUE']?><?endif?>
					</div>
<div itemprop="offers"  itemtype="http://schema.org/Offer" itemscope>
					<?if($arResult['CAN_BUY']=="Y"):?>
					<div class="product_status" itemprop="availability" href="http://schema.org/InStock">есть в наличии</div>
					<?else:?>
                    <div class="product_status not"  itemprop="availability" href="http://schema.org/InStock">нет в наличии</div>
					<?endif?>
...
?>
	<div class="product_price" id="<? echo $arItemIDs['PRICE']; ?>" itemprop="price" content="<?=$arResult['MIN_PRICE']['DISCOUNT_VALUE']?>"><? echo $arResult['MIN_PRICE']['PRINT_DISCOUNT_VALUE']; ?></div><span style='display:none;' itemprop="priceCurrency" content="RUB">RUB</span>
	<?if ((int)$arResult['MIN_PRICE']['VALUE'] < (int)$arResult['PROPERTIES']['OLD_PRICE']['VALUE'] && !empty($arResult['PROPERTIES']['OLD_PRICE']['VALUE']))
		{
			?><div class="product_old_price"><? echo number_format($arResult['PROPERTIES']['OLD_PRICE']['VALUE'], 0, ' ', ' ')?> руб.</div><?
		}?>
...
?>
	...
	<div class="item_buttons vam">
		...
		<span class="item_buttons_counter_block">
			<a href="javascript:void(0);" onclick="SendAnalyticsGoal('kupit_karta');" data-buy="<? echo $arItemIDs['BUY_LINK']; ?>" class="<? echo $buyBtnClass; ?> buy_btn" id="<? echo $arItemIDs['BUY_LINK']; ?>"><span></span><? echo $buyBtnMessage; ?></a>
...
?>
	<div class="product_options">
			...
			<?if($arResult['CAN_BUY']):?><a href="javascript:void(0);" class="product_buy js-action-buy" data-buy="action=buy&id=<?=$arResult['ID']?>" data-id="<? echo $arItemIDs['BUY_LINK']; ?>"><span></span><? echo $buyBtnMessage; ?></a><?else:?><a style="visibility:hidden" href="javascript:void(0);" class="product_buy">Купить</a><?endif?>
			<a href="#" data-link="action=favorites&id=<?=$arResult['ID']?>" class="product_fav js-action-fav">В закладки / отложить</a>
<?
	if ('Y' == $arParams['DISPLAY_COMPARE'] || 1)
	{
	$to_compare = $_SESSION['CATALOG_COMPARE_LIST'][$arResult['IBLOCK_ID']]['ITEMS'][$arResult['ID']];
?>
			<a data-compared="<?=$to_compare?"Y":"N"?>" data-no="Сравнить" data-ok="Сравнить [x]" data-data="id=<?=$arResult['ID']?>&action=<?if($to_compare):?>DELETE_FROM_COMPARE_LIST<?else:?>ADD_TO_COMPARE_LIST&renew=1<?endif?>" id="<? echo $arItemIDs['COMPARE_LINK']; ?>" href="javascript:void(0)" class="product_compare js-compare" style="margin-left: 10px"><? echo ('' != $arParams['MESS_BTN_COMPARE']
					? ($to_compare ? "Сравнить [x]" : $arParams['MESS_BTN_COMPARE'])
					: GetMessage('CT_BCE_CATALOG_COMPARE')
				); ?></a>
<script>
$(function(){
	$(document).on({
		click: function(){
			...
			$.get(url,data);
			if ($(this).data("compared")=="Y") {
				$(this).text($(this).data("no"));
				$(this).data("compared","N");
			}  else {
				$(this).text($(this).data("ok"));
				$(this).data("compared","Y");
			}
			return false;
		}
	},".js-compare");
...
<div class="item_info_section product_description">
<?
...
<?
		foreach ($arResult['DISPLAY_PROPERTIES'] as &$arOneProp)
		{ if($arOneProp['CODE']=="COLOR_REF"||$arOneProp["CODE"]=="VIDEO") continue;
?>
		<li><? echo $arOneProp['NAME']; ?> &mdash; <?
			echo '', (
...
<?if($arResult['PROPERTIES']['COLOR_REF']['VALUE']):?>
<div class="product_icons">
	<?foreach($arResult['PROPERTIES']['COLOR_REF']['VALUE'] as $v): $v=$arResult['ICONS'][$v];?>
...
?>
		...
		<div class="bx_lb">
<div class="tac ovh clearfix">
</div>
<div class="tab-section-container">
<?
if ('Y' == $arParams['USE_COMMENTS0'])
{
?>
<?$APPLICATION->IncludeComponent(
...
?>
<div style="display:none" id="backlink">
	<?$b = $_SERVER['HTTP_REFERER']; if (!$b) $b = $arResult["SECTION"]["SECTION_PAGE_URL"];?>
		<a id="back" class="back" onclick="history.go(-1); return false;" href="<?=$b?>">Вернуться к списку товаров</a>
	<??>
</div>
<script type="text/javascript">
$(function(){
	$(".main").prepend($("#backlink").html());
});
