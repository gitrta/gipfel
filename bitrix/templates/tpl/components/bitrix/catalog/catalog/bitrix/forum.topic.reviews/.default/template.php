            ...
            <div class="reviews_block clearfix">
                <div class="reviews_left">
                    <div class="reviews_title">
                        Отзывы
                        <div class="reviews_title_count"><?=intval($arResult['ELEMENT']['PRODUCT_PROPS']['FORUM_MESSAGE_CNT']['VALUE'])?></div>
                    </div>
...
?>
	<table cellspacing="0" border="0" class="reviews-post-table <?=($iCount == 1 ? "reviews-post-first " : "")?><?
		...
		<tbody><tr><td>
                        ...
                        <div class="reviews_list_item"><a name="message<?=$res["ID"]?>"></a>
                            <div class="reviews_list_item_info">
                                <div class="reviews_list_item_name"><?=$res["AUTHOR_NAME"]?></div>
                                <div class="reviews_list_item_date"><?=$res["POST_DATE"]?></div>
                            </div>
...
?>
	<div style="position:relative; display: block; width:100%;"><div class="reviews_form_title">Добавить отзыв</div>
<?
/* GUEST PANEL */
if (!$arResult["IS_AUTHORIZED"]):
?>		
			<div class="reviews_form_label reviews-reply-field-author"><label for="REVIEW_AUTHOR<?=$arParams["form_index"]?>"><?=GetMessage("OPINIONS_NAME")?><?
				?><span class="reviews-required-field">*</span></label></div>
				<div class="reviews_form_input"><input name="REVIEW_AUTHOR" id="REVIEW_AUTHOR<?=$arParams["form_index"]?>" size="30" type="text" value="<?=$arResult["REVIEW_AUTHOR"]?>" tabindex="<?=$tabIndex++;?>" /></div>
<?		
	if ($arResult["FORUM"]["ASK_GUEST_EMAIL"]=="Y"):
?>
			
			<div class="reviews_form_label"><label for="REVIEW_EMAIL<?=$arParams["form_index"]?>"><?=GetMessage("OPINIONS_EMAIL")?></label></div>
			<div class="reviews_form_input"><input type="text" name="REVIEW_EMAIL" id="REVIEW_EMAIL<?=$arParams["form_index"]?>" size="30" value="<?=$arResult["REVIEW_EMAIL"]?>" tabindex="<?=$tabIndex++;?>" /></div>
<?
	endif;
