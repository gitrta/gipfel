		...
		function ShowComment($comment, $tabCount=0, $tabSize=2.5, $canModerate=false, $User=Array(), $use_captcha=false, $bCanUserComment=false, $errorComment=false, $arParams = array())
		{
			...
			if($comment["SHOW_AS_HIDDEN"] == "Y" || $comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
			{
				...
				if($comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
				{
					...
					?>
					<div class="reviews_list_item 0blog-comment-cont<?=$aditStyle?>">
					<div class="0blog-comment-cont-white">
					<div class="reviews_list_item_info">
						...
						{
							?>
							<div class="reviews_list_item_name"><?=$comment["AuthorName"]?></div>
							<?
						}
						...
						?>
						<div class="reviews_list_item_date"><?=$comment["DateFormated"]?></div>
					</div>
		...
		if($arResult["is_ajax_post"] != "Y")
		{
			...
			if($arResult["NEED_NAV"] == "Y")
			{
				?>
					...
					{
						$style = "";
						//if ($i==$arResult["PAGE_COUNT"]) $style.=" last";
						if($i == $arResult["PAGE"])
							$style .= " active";
						?><li class="<?=$style?>"><a href="<?=$arResult["NEW_PAGES"][$i]?>" onclick="return bcNav('<?=$i?>', this)" id="blog-comment-nav-t<?=$i?>"><?=$i?></a></li><?
					}
				?>
				</ul>
				<?
                ...
                <div class="reviews_left">
					<div id="blog-comment-page-<?=$i?>"<?if($arResult["PAGE"] != $i) echo "style=\"display:none;\""?>>
					  <div class="reviews_title">
                        Отзывы
                        <div class="reviews_title_count"><pre><?=intval($arResult["Post"]['NUM_COMMENTS'])?></pre></div>
                    </div>
...
<?if($_REQUEST['act']!="add"):?>
                ...
                <div class="reviews_right">
                    <div class="reviews_form"><div class="reviews_form_title">Добавить отзыв</div>
<?
		if($arResult["is_ajax_post"] != "Y" && $arResult["CanUserComment"])
		{
			...
			<div id="form_comment_">
				...
				<div class="blog-comment-fields00">
					<?
					if(empty($arResult["User"]))
					{
						?>
                            ...
                            <div class="reviews_form_label">Ваше имя</div>
                            <div class="reviews_form_input">
                                <input type="text"  name="user_name" id="user_name" value="<?=htmlspecialcharsEx($_SESSION["blog_user_name"])?>">
                            </div>
                            <div class="reviews_form_label">Электронная почта</div>
                            <div class="reviews_form_input">
                                <input type="text" tabindex="4" type="text" name="user_email" id="user_email" value="<?=htmlspecialcharsEx($_SESSION["blog_user_email"])?>">
                            </div>						
						<?
					}
					?>
					...
					?>
                            <div class="reviews_form_label">Ваш отзыв</div>
                            <div class="reviews_form_input">
					...
					?>
                            <div class="review_form_submit">
                                <input tabindex="10" type="button" value="Добавить отзыв" name="sub-post" id="post-button" onclick="submitComment()">
                            </div>
				</div>
        ...
        <div class="wrapper">
            <div class="reviews_block clearfix">
                <div class="reviews_left">
					  <div class="reviews_title">
                        Отзывы 
                        <div class="reviews_title_count"><?=(int)$arResult['Post']['NUM_COMMENTS']?></pre></div>
                    </div>
                    <div class="reviews_list">						
					<div id="new_comment_cont_0"></div>
					<div id="new_comment_0" style="display:none;"></div>	
<?endif?>					
			...
			<div id="form_comment_">
				<div id="form_c_del">
				<div class="blog-comment-form">
				<div class="reviews_form_title">Добавить отзыв</div>
				<form method="POST" name="form_comment" id="form_comment" action="<?=$templateFolder."/ajax.php"?>">
				<input type="hidden" name="parentId" id="parentId" value="">
				<input type="hidden" name="edit_id" id="edit_id" value="">
				<input type="hidden" name="act" id="act" value="add">
