...
JCSmartFilter.prototype.postHandler = function (result)
{
    ...
    if(!!result && !!result.ITEMS)
    {
        ...
        if(modef && modef_num)
        {
            $("#chosen span").text(result.ELEMENT_COUNT);
            $("#chosen").show();
            ...
            {
                var hrefs = document.getElementsByClassName('price_slider_button');
                var href = result.FILTER_URL.replace(/&amp;/g, "&");
                for(i = 0; i < hrefs.length; i++){
                    hrefs[i].setAttribute('href', href);
                }
...
cDoubleTrackBar.prototype = {
    ...
    changeInput: function(){
        ...
        this.MinInputId.onkeyup = function(){
            if(that.MinInputId.value > that.Min || that.MinInputId.value < that.Max ) {
                that.startPosition();
                smartFilter.keyup(that.MinInputId);
            }
        };

        this.MaxInputId.onkeyup = function(){
            if(that.MaxInputId.value > that.Min || that.MaxInputId.value < that.Max ) {
                that.startPosition();
                smartFilter.keyup(that.MaxInputId);
            }
        };
    ...
    startPosition: function ()
    {
        ...
        if (curMinPrice || curMaxPrice)
        {
            ...
            var koef = ((this.Max - this.Min)*100000/drag_width)/100000;
            var percent_koef = drag_width/100;
            if (curMinPrice)
                var curLeft = (curMinPrice - this.Min)/koef;
            if (curMaxPrice)
                var curWidth = (curMaxPrice - this.Min)/koef;
            ...
            if (curWidth/percent_koef <= 100)
                this.RightDrag.style.left = (curWidth-12)/percent_koef  + "%";
        }
    }
}
