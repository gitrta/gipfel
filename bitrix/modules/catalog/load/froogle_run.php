...
/*		if ($GLOBALS["APPLICATION"]->GetFileAccessPermission($SETUP_FILE_NAME) < "W")
	...
	if (strlen($strExportErrorMessage)<=0)
	{
		...
		{
			if (!@fwrite($fp, "id	link	title	description	image_link	category	availability	price\n"))
			{
	...
	if (strlen($strExportErrorMessage)<=0)
	{
		...
		while ($ar_elems = $db_elems->GetNext())
		{
			...
			if (strpos($minPrice, ".") !== false) {
				$minPrice .= " RUB";
			}else{
				$minPrice .= ".00 RUB";
			}

			$description = PrepareString($ar_elems["~PREVIEW_TEXT"], true);
			if(!strlen(trim($description))){
				$description = $ar_elems["~NAME"];
			}

			@fwrite($fp, $ar_elems["ID"].
				"	".
				"https://".
