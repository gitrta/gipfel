			...
			foreach ($itemIdsList as $id)
			{
				...
				if ($row['CATALOG_TYPE'] == Catalog\ProductTable::TYPE_SKU && !empty($row['OFFERS']))
				{
					...
					foreach ($row['OFFERS'] as $offer)
					{
						...
						//$referer = '?utm_source=cpc_yandex_market&amp;utm_medium=cpc&amp;utm_campaign='.$row['CATEGORY_ID'].'&amp;utm_content='.yandex_text2xml($offer['NAME'], true).'&amp;utm_term='.$offer['ID'].'';
						//$referer = '?utm_source=cpc_yandex_market&amp;utm_medium=cpc&amp;utm_campaign='.$row['CATEGORY_ID'].'&amp;utm_content='.yandex_text2xml(urlencode($offer['NAME']), true).'&amp;utm_term='.$offer['ID'].'';
						$referer = '?utm_source=cpc_yandex_market&amp;utm_medium=cpc&amp;utm_campaign='.$row['CATEGORY_ID'].'&amp;utm_term='.$offer['ID'].'';
						/*if (!$disableReferers)
							$referer = (strpos($offer['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;').'r1=<?=$strReferer1; ?>&amp;r2=<?=$strReferer2; ?>';*/

						$itemsContent .= "<url>".$usedProtocol.$ar_iblock['SERVER_NAME'].htmlspecialcharsbx($offer['DETAIL_PAGE_URL'])."</url>\n";
						unset($referer);
				...
				elseif (isset($simpleIdsList[$id]) && !empty($row['PRICES']))
				{
					...
					/*$referer = '';
					if (!$disableReferers)
						$referer = (strpos($row['DETAIL_PAGE_URL'], '?') === false ? '?' : 'с').'r1=<?=$strReferer1; ?>сяяr2=<?=$strReferer2; ?>';*/
					//$referer = '?r1=yandext&amp;utm_source=cpc_yandex_market&amp;utm_medium=cpc&amp;utm_campaign='.$row['CATEGORY_ID'].'&amp;utm_content='.yandex_text2xml(urlencode($row['NAME']), true).'&amp;utm_term='.$row['ID'].'';
					$referer = '?utm_source=cpc_yandex_market&amp;utm_medium=cpc&amp;utm_campaign='.$row['CATEGORY_ID'].'&amp;utm_term='.$row['ID'].'';

					$itemsContent .= "<url>".$usedProtocol.$ar_iblock['SERVER_NAME'].htmlspecialcharsbx($row['DETAIL_PAGE_URL'])."</url>\n";
					unset($referer);
