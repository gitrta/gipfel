...
$file = file_get_contents($_SERVER['DOCUMENT_ROOT'].'/uaprice.csv'); //ОСНОВНОЙ CSV ФАЙЛ
$r1 = explode("\r",$file);
//array_shift($r1);
...
foreach($YES as $item){$dp++;
	...
	$PRICE_TYPE_ID = 3;
	$price = str_replace(" ","",full_trim($item[1]));
	$price = str_replace("грн.","",full_trim($price));
	$price = str_replace(",00","",full_trim($price));
	$qty = $item[3];
	$rs = CCatalogStoreProduct::GetList(false, array('PRODUCT_ID'=> $PRODUCT_ID, 'STORE_ID' => 3));
