...
if($SCID){
    ...
    $calc->addTariffPriority($SP2); // Рассчитать для всех
...
$almost = array();
foreach($SD_CODES as $key=>$item){
	$almost['BITRIX_ID']=$item['ID'];
	$almost['NAME']=$item['NAME'];
	$almost['API'][]=$FINALDESTINATIONS[$key];
}
